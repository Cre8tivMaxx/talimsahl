...
- **`--navbar-height` is a trap CSS var in this app (2026-06-16):** desk-overrides.css set `--navbar-height: 56px` (leftover from the reverted navbar.js shell), but Frappe core's `.form-tabs-sticky-down { top: navbar-height - 2px }` (form.scss) uses it to dock the sticky form-tab bar. Since v16 Desk here has no real top navbar, that 56px made the tab bar float 54px into the document and swallow whatever field happened to scroll through that band (bug-042). Fixed by setting it to `0px`. Any `var(--navbar-height|--page-head-height)` consumer in Frappe core (`form.scss` sticky tabs, `desktop.scss`/`kanban.scss` height calc, `page.scss` scroll-margin) must be re-checked whenever a reskin removes/changes the real navbar's footprint.
- **`render()` + `after_render()` are only chained inside `refresh()`'s server-fetch callback (2026-06-16, base_list.js:518-535)** — there is no standalone "re-render from already-fetched data" method. To force a cheap in-memory re-render (e.g. after a client-side view-mode toggle) without a network round trip, call `listview.render(); listview.after_render();` directly. Calling `refresh()` for a pure display toggle causes a double render (one instant, one after the fetch) — visible as content flashing/hiding then lagging back in (bug-043, `list_v2.js` card/row switcher).
